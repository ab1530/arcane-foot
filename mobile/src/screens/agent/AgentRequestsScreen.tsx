...
type AgentRequestsScreenProps = NativeStackScreenProps<AppStackParamList, 'AgentRequests'>;
...
const STATUS_ORDER: AgentRequestStatus[] = ['CREATED', 'IN_PROGRESS', 'SATISFIED', 'CANCELLED'];
const CATEGORY_OPTIONS: Array<AgentRequestCategory> = ['EQUIPMENT', 'INJURY', 'MEDICAL', 'OTHER'];

const canManageStatus = (role?: string | null) => isCategoryARole(role as any) || isCategoryBRole(role as any);
...
const listToCsv = (values?: string[]) => (Array.isArray(values) ? values.join(', ') : '');
...
export default function AgentRequestsScreen() {
  ...
  const [marketRules, setMarketRules] = useState<AgentRequestMarketProfileRule[]>([]);
  ...
  const refresh = useCallback(async () => {
    setRefreshing(true);
    await Promise.all([loadRequests(), loadRules()]);
  }, [loadRequests, loadRules]);

  useEffect(() => {
    void Promise.all([loadRequests(), loadRules()]);
  }, [loadRequests, loadRules]);
  ...
  const handleSubmit = async () => {
    ...
    const payload: AgentRequestCreatePayload = {
      ...
      preferredFoot: form.preferredFoot === 'LEFT' || form.preferredFoot === 'RIGHT' || form.preferredFoot === 'BOTH'
        ? form.preferredFoot
        : undefined,
    };
  ...
  const handleStatusUpdate = async (requestId: string, status: AgentRequestStatus) => {
    try {
  ...
  const handleRuleChange = (ruleId: string, isActive: boolean) => {
    setDraftRules((current) =>
      current.map((rule) => (rule.id === ruleId ? { ...rule, isActive } : rule)),
    );
  };
  ...
  const handleRuleSave = async () => {
    if (!isCategoryARole(role as any)) {
      Alert.alert('Droits insuffisants', 'Seuls les admins peuvent modifier les règles de marché.');
      return;
    }
  ...
  const renderRequest = ({ item }: { item: AgentRequestItem }) => (
    <View style={styles.card}>
      <View style={styles.cardHeader}>
        ...
        <Text style={styles.statusChip}>{statusLabel[item.status] || item.status}</Text>
      </View>
      <Text style={styles.metaText}>
        {categoryLabel[item.category]} • Priorité {item.priority} • Créé le {normalizeDate(item.createdAt)}
      </Text>
      {!!item.dueAt && <Text style={styles.metaText}>Échéance {normalizeDate(item.dueAt)}</Text>}
      {!!item.details && <Text style={styles.bodyText}>{item.details}</Text>}
      {!!item.content?.equipment && <Text style={styles.metaText}>Équipement: {item.content.equipment}</Text>}
      {!!item.content?.medicalDetails && <Text style={styles.metaText}>Médical: {item.content.medicalDetails}</Text>}
      <Text style={styles.metaText}>Demandeur: {item.creator?.firstName} {item.creator?.lastName}</Text>
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      >
        <View style={styles.header}>
          <Text style={styles.title}>{t('dashboard.cardMap.agentRequests') || 'Demandes à l’agent'}</Text>
          ...
          {(['ALL', ...CATEGORY_OPTIONS] as const).map((category) => {
            ...
            return (
              <TouchableOpacity
                ...
                onPress={() => setFilters((current) => ({ ...current, category: category as any }))}
              >
                <Text style={[styles.filterChipText, isActive && styles.filterChipTextActive]}>
                  {category === 'ALL' ? 'Toutes' : categoryLabel[category as AgentRequestCategory]}
                </Text>
              </TouchableOpacity>
            );
          })}
          ...
          {(['ALL', ...STATUS_ORDER] as const).map((status) => {
            ...
            return (
              <TouchableOpacity
                ...
                onPress={() => setFilters((current) => ({ ...current, status: status as any }))}
              >
                <Text style={[styles.filterChipText, active && styles.filterChipTextActive]}>
                  {status === 'ALL' ? 'Tous' : statusLabel[status as AgentRequestStatus]}
                </Text>
              </TouchableOpacity>
            );
          })}
        ...
        <View style={styles.formCard}>
          ...
          <TextInput
            ...
            placeholderTextColor={theme.colors.text.secondary}
            value={form.playerId}
            onChangeText={(value) => setForm((current) => ({ ...current, playerId: value }))}
          />
          <TextInput
            style={styles.input}
            ...
            placeholderTextColor={theme.colors.text.secondary}
            value={form.dueAt}
            onChangeText={(value) => setForm((current) => ({ ...current, dueAt: value }))}
          />
          {form.category === 'EQUIPMENT' ? (
            <TextInput
              ...
              onChangeText={(value) => setForm((current) => ({ ...current, equipment: value }))}
            />
          ) : null}
          {form.category === 'MEDICAL' ? (
            <TextInput
              ...
              onChangeText={(value) => setForm((current) => ({ ...current, medicalDetails: value }))}
            />
          ) : null}
          <TouchableOpacity
            style={styles.submit}
        ...
        {renderSectionHeader(`Demandes en cours (${pendingRequestsCount})`)}
        <FlatList
          data={requests}
          ...
          scrollEnabled={false}
          ListEmptyComponent={
            <Text style={styles.emptyState}>Aucune demande pour ce filtre.</Text>
          }
        />
            ...
            <View key={rule.id} style={styles.ruleRow}>
              <View style={styles.ruleContent}>
                ...
                <Text style={styles.ruleMeta}>
                  {rule.market}
                  {rule.positions?.length ? ` • ${rule.positions.join(', ')}` : ''}
                  {typeof rule.minHeightCm === 'number' ? ` • ${rule.minHeightCm} cm min` : ''}
                  {typeof rule.minEndurance === 'number'
                    ? ` • Endurance ${rule.minEndurance}`
                    : ''}
                  {rule.preferredFoot ? ` • Pied ${rule.preferredFoot}` : ''}
                  {rule.traits?.length ? ` • Traits ${rule.traits.join(', ')}` : ''}
                </Text>
                {isCategoryARole(role as any) ? (
                  <View style={styles.ruleEditor}>
                    <TextInput
                      ...
                      onChangeText={(value) =>
                        handleRuleFieldChange(rule.id, { positions: csvToList(value) })
                      }
                    />
                    <TextInput
                      style={styles.ruleInput}
                      ...
                      onChangeText={(value) =>
                        handleRuleFieldChange(rule.id, { traits: csvToList(value) })
                      }
                    />
                    <View style={styles.ruleEditorRow}>
                      <TextInput
                        ...
                        value={typeof rule.minHeightCm === 'number' ? String(rule.minHeightCm) : ''}
                        onChangeText={(value) =>
                          handleRuleFieldChange(rule.id, {
                            minHeightCm: value.trim() ? Number(value) : undefined,
                          })
                        }
                      />
                      <TextInput
                        ...
                        placeholderTextColor={theme.colors.text.secondary}
                        keyboardType="numeric"
                        value={typeof rule.minEndurance === 'number' ? String(rule.minEndurance) : ''}
                        onChangeText={(value) =>
                          handleRuleFieldChange(rule.id, {
                            minEndurance: value.trim() ? Number(value) : undefined,
                          })
                        }
                      />
                    </View>
                    <View style={styles.ruleFootRow}>
                      {(['LEFT', 'RIGHT', 'BOTH'] as const).map((foot) => {
                        ...
                        return (
                          <TouchableOpacity
                            key={`${rule.id}-${foot}`}
                            style={[styles.ruleFootChip, active && styles.ruleFootChipActive]}
                            onPress={() =>
                              handleRuleFieldChange(rule.id, {
                                preferredFoot: foot as AgentRequestMarketProfileRule['preferredFoot'],
                              })
                            }
                          >
                            <Text
              ...
              {isCategoryARole(role as any) ? (
                <Switch
                  value={!!rule.isActive}
                  onValueChange={(isActive) => handleRuleChange(rule.id, isActive)}
                  trackColor={{ true: theme.colors.brand.primary, false: theme.colors.semantic.muted }}
                  thumbColor={theme.colors.text.primary}
                />
              ) : (
                <Text style={styles.ruleReadOnlyChip}>{rule.isActive ? 'Actif' : 'Inactif'}</Text>
              )}
            </View>
          ))}
          {isCategoryARole(role as any) ? (
            <TouchableOpacity style={styles.submit} onPress={handleRuleSave} activeOpacity={0.85}>
              {saving ? (
                <ActivityIndicator color={theme.colors.text.primary} />
              ) : (
